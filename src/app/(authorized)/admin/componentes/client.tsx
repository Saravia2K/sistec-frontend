...
type TComponentesPageClientProps = {
  data: TComponent[];
};

export default function ComponentesPageClient({ data }: TComponentesPageClientProps) {
  const { components, componentsLoading, reloadComponents } = useComponents(data);
  const [openForm, setOpenForm] = useState(false);
  const [editingComponent, setEditingComponent] = useState<TComponent | null>(null);
  ...
  const handleToggleVisibility = async (component: TComponent) => {
    try {
      ...
      reloadComponents();
      toast(`Componente ${!component.visible ? "visible" : "oculto"} correctamente`, {
        type: "success",
      });
    } catch (error) {
                    ...
                    <TableRow key={component.id}>
                      ...
                      <TableCell>
                        <Box display="flex" gap={1}>
                          <Button color="green" onClick={() => handleOpenForm(component)}>
                            Editar
                          </Button>
                          {/* <Button
                            color="blue"
